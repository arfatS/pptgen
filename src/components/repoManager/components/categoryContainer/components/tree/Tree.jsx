...
class Tree extends React.Component {
  ...
  onNodeDragEnter = (event, node) => {
    ...
    clearTimeout(this.delayedDragOverLogic);
    this.delayedDragOverLogic = setTimeout(() => {
      this.setState({
  ...
  intervalStartStop = (flag, direction) => {
    const {
      isIntervalStart
    } = this.state;
  ...
  onNodeDragOver = (event, node) => {
    ...
    const { isIntervalStart } = this.state;
    // Update drag position
    let e = event;
    if (e.clientY > (window.innerHeight - 150)) {
      if (this.state.scrollDirection !== "downFast") {
        if(isIntervalStart) {
          this.intervalStartStop(false);
        }
        
        window.scrollBy(0, 30);
        this.intervalStartStop(true, "downFast");
      }
      ...
      if (this.state.scrollDirection !== "upFast") {
        ...
        window.scrollBy(0, -30);
        this.intervalStartStop(true, "upFast");
      }
    } else if(isIntervalStart) {
      this.setState({
        isIntervalStart: false
      }, () => {
        this.intervalStartStop(false);
  ...
  onNodeSelect = (e, treeNode) => {
    ...
    if (onSelect) {
      const eventObj = {
        ...
        targetKey: arrAdd(selectedKeys, eventKey), //target key when deselected
        children: treeNode.getNodeChildren() //children
      };
