  ...
  if (response && response.status) {
    ...
    if ((responseData && responseData.success) || statusCodeRange) {
      formattedResponse = {
        success: true,
        status: 200,
        data: responseData && responseData.data ? responseData.data : null
      };
    } else {
      formattedResponse = {
        success: false,
...
class FetchUtils {
  ...
  postData = async (url, body, log) => {
    try {
      ...
      logError({ error, extraData: error.response });
      return handleReponseConvert({
        error
      });
    }
  ...
  getDataWithHeader = async (url, log, customHeader = {}) => {
    const apiUrl = this.baseUrl + url;
    try {
      const headers = this.setHeaders(customHeader);
